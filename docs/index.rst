...
Then we specify our dataset, here it is `ClassIncremental` (*New Classes* NC) where each task
...
The continual loader, here named `clloader` is an iterable. Each loop provides then
...
From this loader, we can get the first task `clloader_test[0]`, all tasks up to the third
task `clloader_test[:3]`, or even all tasks `clloader_test[:]`. You can slice
