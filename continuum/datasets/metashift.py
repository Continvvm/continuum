...
class MetaShift(_ContinuumDataset):
    ...
    def __init__(
        ...
        class_names: Union[
            Iterable[str], None
        ] = None,  # Only get images corresponding to specific classe(s)
        context_names: Union[
            Iterable[str], None
        ] = None,  # Only get images corresponding to specific context(s)
        unique_occurence: bool = False,
        # If true, the images will be assigned random class(context) combination among all valid combinations.
        random_seed: int = 42,
        nb_tasks: int = 0,
        ...
        self.visual_genome_path = (
            os.path.expanduser(visual_genome_path)
            if visual_genome_path is not None
            else os.path.join(os.path.expanduser(data_path), "MetaShift", "images")
        )
    ...
    def get_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        with open(
            os.path.join(self.data_path, "full-candidate-subsets.pkl"), "rb"
        ) as pkl_file:
        ...
        for key in pkl_dict.keys():
            ...
            if (self.class_names is None or class_name in self.class_names) and (
                self.context_names is None or context_name in self.context_names
            ):
    ...
    def get_class_context_in_order(self):
        ...
        if self.class_names_updated is None or self.context_names_updated is None:
            warn(
                "get_class_context_in_order, the class and task indices are not known yet : get_data() was not called and no scenario was created."
            )
    ...
    def _strict_domain_tasks(self, x, y, t):
        ...
        if np.all(selected_tasks == 0):
            raise ValueError(
                "Error : No task contains all classes. Try with fewer classes or set strict_domain_inc to false."
            )
