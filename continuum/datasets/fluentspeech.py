...
class FluentSpeech(_AudioDataset):
    ...
    def __init__(
        self, data_path, train: Union[bool, str] = True, download: bool = True
    ):
        if not isinstance(train, bool) and train not in ("train", "valid", "test"):
            raise ValueError(
                f"`train` arg ({train}) must be a bool or train/valid/test."
            )
    ...
    def _download(self):
        if not os.path.exists(
            os.path.join(self.data_path, "fluent_speech_commands_dataset")
        ):
            tgz_path = os.path.join(
                self.data_path, "jf8398hf30f0381738rucj3828chfdnchs.tar.gz"
            )
    ...
    @property
    def locations(self):
        return {"none": 0, "kitchen": 1, "bedroom": 2, "washroom": 3}
