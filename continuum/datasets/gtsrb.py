...
class GTSRB(ImageFolderDataset):
    ...
    * Detection of Traffic Signs in Real-World Images: The {G}erman {T}raffic {S}ign {D}etection {B}enchmark}
    ...
    def _download(self):
        ...
        urls = [self.train_images_url, self.test_images_url, self.test_gt_url]
    ...
    def get_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        if self.train:
            dataset = torchdata.ImageFolder(
                os.path.join(self.data_path, "GTSRB", "Final_Training", "Images")
            )
        ...
        with open(os.path.join(self.data_path, "GT-final_test.csv")) as f:
            ...
            for line in f:
                ...
                x.append(
                    os.path.join(
                        self.data_path, "GTSRB", "Final_Test", "Images", line[0]
                    )
                )
                y.append(int(line[-1]))
