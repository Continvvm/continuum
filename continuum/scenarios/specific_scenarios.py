...
class ALMA(InstanceIncremental):
    ...
    def __init__(
        ...
        if nb_megabatches is None:
            raise Exception(
                "User must specify the number of mega-batches in the stream"
            )

        self.alma_random_seed = random_seed
        super().__init__(
            cl_dataset=cl_dataset,
            nb_tasks=nb_megabatches,
    ...
    def _setup(self, nb_tasks: Optional[int]) -> int:
        ...
        if len(y) < nb_tasks:
            raise Exception(
                "Cannot have more tasks than available samples in the dataset"
            )
        ...
        if nb_tasks > 0:
            ...
            raise Exception(
                f"You must then specify a positive number of tasks, not {nb_tasks}."
            )
