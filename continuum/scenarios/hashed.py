...
class HashedScenario(ContinualScenario):
    ...
    def __init__(
        ...
        if self.split_task not in ["balanced", "auto"]:
            AssertionError(
                f"{self.split_task} is not a data_split parameter available."
            )
        if split_task == "balanced" and nb_tasks is None:
            AssertionError(
                f"self.data_split is {self.split_task} the nb_tasks should be set."
            )
    ...
    def hash_func(self, x):
        ...
        if self.hash_name == "AverageHash":
            ...
            hash_value = imagehash.whash(
                x, hash_size=8, image_scale=None, mode="haar", remove_max_haar_ll=True
            )
            ...
            hash_value = imagehash.crop_resistant_hash(
                x,
                hash_func=None,
                limit_segments=None,
    ...
    def get_task_ids(self, x):
        ...
        if self.split_task == "balanced":
            ...
            perfect_balance_task_ids = np.arange(self._nb_tasks).repeat(
                example_per_tasks
            )
            ...
            task_ids = None
            while bandwidth > 0.5:
                task_ids = MeanShift(bandwidth=bandwidth, bin_seeding=True).fit_predict(
                    reduc_data
                )
    ...
    def generate_task_ids(self, cl_dataset):
        ...
        if self.filename_hash_indexes is not None and os.path.exists(
            self.filename_hash_indexes
        ):
            print(
                f"Loading previously saved sorted indexes ({self.filename_hash_indexes})."
            )
            ...
            if self.filename_hash_indexes is not None:
                np.save(
                    self.filename_hash_indexes,
                    [task_ids, sort_indexes, vectorized_list_hash],
                    allow_pickle=True,
                )
