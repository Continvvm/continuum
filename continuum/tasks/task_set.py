...
def TaskSet(
    ...
    if data_type == TaskType.TEXT:
        assert bounding_boxes is None, print(
            "bounding_boxes are not compatible with TaskType.TEXT"
        )
        task_set = TextTaskSet(x=x, y=y, t=t, trsf=trsf, target_trsf=target_trsf)
    elif data_type == TaskType.IMAGE_ARRAY:
        task_set = ArrayTaskSet(
            x=x,
            y=y,
            t=t,
            trsf=trsf,
        ...
        assert bounding_boxes is None, print(
            "bounding_boxes are not compatible with TaskType.TENSOR"
        )
